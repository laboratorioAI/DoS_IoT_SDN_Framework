...
do
	...
	sleep 2s
	sudo ffmpeg -r 24 -i /data/Descargas/media_org/media/bunny.mp4 -r 24 -i /data/Descargas/media_org/media/stream/mininet_video_h7_1234_$i.mp4 -lavfi  "[0:v]setpts=PTS-STARTPTS[reference]; [1:v]scale=320:240:flags=bicubic,setpts=PTS-STARTPTS[distorted]; [distorted][reference]libvmaf=psnr=1:ssim=1:log_fmt=csv:model_path=/usr/local/share/model/vmaf_v0.6.1.pkl:log_path=/data/Descargas/media_org/dataset/datos/logpr$i.csv" -f null -
	sleep 15s
